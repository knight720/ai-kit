...
gws docs documents get --params "{`"documentId`": `"$docId`", `"includeTabsContent`": true}" > $rawFile
```

> **注意：** 使用 `> $rawFile` 重導向到檔案，不要直接在 terminal 顯示（會因 CP950 亂碼）。
...
# Fix 2: 狀態機 — 將 JSON 字串值內的 literal 控制字元 escape 掉
def fix_json(s):
...
# Fix 3: 過濾 Unicode Private Use Area 字元（字型私用符號）
PUA = re.compile(r'[\ue000-\uf8ff]')
...
```powershell
gws docs documents get --params '{"documentId": "1WR9hSVEFKjGlYZQcZT4o9vKrVceqeauDVQGWGPPRvBM"}' > $env:TEMP\gdoc_raw.txt
```
...
| 內容包含大量 `?` | 原始文件有 `errors='replace'` 替換 | 確認 raw 檔 encoding，改用 `errors='ignore'` |
